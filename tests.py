import pytest
from library_system_modules.library_system import Book, BookGenre, BookNotAvailableError, LateReturnError
...
def test_invalid_membership():
    with pytest.raises(InvalidMembershipError) as excinfo:
        member = Member("Eve", "INVALID_LEVEL")
        member.get_fee()
    assert str(excinfo.value) == "Invalid membership level: INVALID_LEVEL"
